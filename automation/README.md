...
> Heads-up: posts are auto-published. Skim them now and then — if one reads thin,
> just delete that entry from `blog.json` and tighten the topic's brief in the bank.

# Auto slide decks (visual explainers)

Blogs are text-heavy, so there's also a **Slides** section — in-depth, swipeable,
LinkedIn-carousel-style concept explainers (**15–20 slides** each). Slides are
**structured JSON**, not images: each slide is a typed layout block (table, code,
flowchart, comparison, before/after metrics, numbered cards, callout…) rendered by
`SlideView.jsx` into a polished card that adapts to light/dark. Generated by a second
**Cowork scheduled task**, `portfolio-slides`, **1 per day** (~12:30).

How the pieces fit:
- **Scheduled task `portfolio-slides`** — picks the next topic from
  `automation/slides-topics.json`, writes a 15–20 slide structured deck, and PREPENDS
  it to `portfolio/src/data/slides.json` (then marks the topic used). No git.
- **`slides-topics.json`** — the curated deck topic bank (request flow, RAG, TLS,
  indexing, OAuth, caching, rate limiting, etc.).
- The same **`publish.sh`** publisher deploys decks too (it commits *all* content
  changes), so no extra setup beyond the blog publisher above.

Data shape: a deck is `{ id, title, excerpt, tags, date, slides:[...] }`. A slide is
`{ tag, title, intro?, body }` where `body.kind` is one of: `cover, callout, bullets,
numbered, steps, decision, table, code, metrics, compare`. The renderer styles every
kind with the site's tokens, so decks always look on-brand. Two deep starter decks
ship in `slides.json` (Database Indexing, Caching). `SlideView.jsx` also still renders
legacy `{ svg }` / `{ image }` slides, so you can drop in a real screenshot/diagram on
any individual slide if you want.

Replenish `slides-topics.json` the same way as the blog bank when it runs low.
